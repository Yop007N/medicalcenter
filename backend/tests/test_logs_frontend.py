...
class TestFrontendLogsEndpoints:
    ...
    def test_receive_frontend_logs_success(self, client, tmp_path, monkeypatch):
    ...
    def test_receive_frontend_logs_validates_payload(self, client):
        ...
        non_object = client.post('/api/logs/frontend', json=['not-object'])
        assert non_object.status_code == 400
        assert non_object.json['msg'] == 'Request body must be an object'

        invalid_logs_type = client.post('/api/logs/frontend', json={'logs': {}})
        assert invalid_logs_type.status_code == 400
        assert invalid_logs_type.json['msg'] == 'logs must be an array'

    def test_receive_frontend_logs_internal_error_is_sanitized(self, client, monkeypatch):
        def _raise_io_error(*_args, **_kwargs):
            raise OSError('disk full')

        monkeypatch.setattr(builtins, 'open', _raise_io_error)

        response = client.post('/api/logs/frontend', json={'logs': [{'message': 'will fail'}]})
