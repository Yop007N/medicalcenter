...
def check_file_access(file_record, user_id):
    ...
    # Check medical record association
    medical_record = file_record.medical_record
    if not medical_record:
        # Orphaned file? Only admin or maybe uploader
        return file_record.uploaded_by == user_id

    if user.role == "professional":
        # Professional can access if they are the one in the record
        if medical_record.professional_id == user.id:
            return True

    if user.role == "patient":
        # Patient can access if it is their record
        if medical_record.patient_id == user.id:
            return True

    # Fallback: if user uploaded the file
...
def upload_file():
    ...
    parameters:
      - in: formData
        ...
        type: file
        required: true
        description: Archivo médico (pdf, png, jpg, gif, dcm, doc, docx)
      - in: formData
    ...
    if user.role == "admin":
        ...
        user.role == "professional" and
        medical_record.professional_id == current_user_id
    ):
        is_authorized = True
    elif (
        user.role == "patient" and
        medical_record.patient_id == current_user_id
    ):
        is_authorized = True
