...
def check_file_access(file_record):
    """
    ...
    # Admins and Professionals can access all files
    if user.role in ['admin', 'professional']:
        return True

    # Patients can only access files linked to their medical record
    medical_record = MedicalRecord.query.get(file_record.medical_record_id)
    if user.role == 'patient' and medical_record and medical_record.patient_id == current_user_id:
        return True
...
def get_file(file_id):
    ...
    if not check_file_access(file_record):
