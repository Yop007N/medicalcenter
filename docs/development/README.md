# Desarrollo - Documentacion Activa

Actualizado: 2026-03-01

## Documentos activos
- `solid_activity_tracker.md`: estado por modulo con evidencia ejecutable.
- `SPRINT_BACKLOG_P3_PARALLEL_2026-02-26.md`: backlog grande vigente por tracks A/B/C/D/E.
- `UC_RF_VERIFICATION_2026-02-24.md`: matriz de requisitos/casos de uso verificados.
- `setup.md`: setup de entorno.
- `coding-standards.md`: convenciones de desarrollo.
- `WAVE_EXECUTION_AUTONOMA.md`: operacion por olas con orquestador.

## Documentos archivados (no vigentes)
Se movieron a `docs/archive/legacy/development/`:
- `SPRINT_BACKLOG_P0_P1_P2_2026-02-24.md`
- `solid_parallel_backlog_2026-02-24.md`

## Regla operativa
- No crear nuevos reportes/versiones dentro del repo para outputs efimeros (logs, reportes de test, reportes Playwright).
- Usar `solid_activity_tracker.md` como registro unico de evidencia.
